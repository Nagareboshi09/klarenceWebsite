          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mb-24">
            ...
            >
              ...
              <div className="space-y-4 text-muted-foreground">
                ...
                <p>
                  Beyond coding, I have practical experience in networking, database administration, and cybersecurity fundamentals. I thrive on solving complex problems, adapting to new tech stacks, and turning paper-based challenges into seamless digital workflows.
                </p>
              </div>
            </motion.div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <InfoCard icon={User} title="Personal Interests">
                <ul className="space-y-2 text-muted-foreground">
                  {["Exploring new technologies", "Likes solving problems", 
                    "Open minded", "Web development/Mobile projects"].map((interest, i) => (
                    <motion.li 
                      key={i} 
              ...
              <InfoCard icon={Code} title="Coding Philosophy">
                <ul className="space-y-2 text-muted-foreground">
                  {["Clean and maintainable code", "Focus on user experience", 
                    "Continuous learning and improvement", "Building Accessible website for everyone"].map((philosophy, i) => (
                    <motion.li 
                      key={i} 
                      className="flex items-center"
                      whileHover={{ x: 5, color: "#9b87f5" }}
              ...
              <InfoCard icon={GraduationCap} title="Education Journey">
                <ul className="space-y-2 text-muted-foreground">
                  {["Java fundamentals", "Data structures and algorithms", 
                    "Cyber Security Privacy and Security Fundamentals", "Self-taught web development/Mobile Development"].map((journey, i) => (
                    <motion.li 
                      key={i} 
                      className="flex items-center"
                      whileHover={{ x: 5, color: "#9b87f5" }}
              ...
              <InfoCard icon={BookOpen} title="Future Goals">
                <ul className="space-y-2 text-muted-foreground">
                  {["Mastering advanced AI techniques", "Creating AI LLM solution for the clients", 
                    "Contributing to open-source projects", "Exploring new language trends"].map((goal, i) => (
                    <motion.li 
                      key={i} 
                      className="flex items-center"
                      whileHover={{ x: 5, color: "#9b87f5" }}
